...
int main(int argc, char *argv[]) {
    ...
    const string veto_panel_names[10] = {
        "Veto_Panel_12", "Veto_Panel_13", "Veto_Panel_14", "Veto_Panel_15",
        "Veto_Panel_16", "Veto_Panel_17", "Veto_Panel_18", "Veto_Panel_19",
        "Veto_Panel_20", "Veto_Panel_21"
    };
    for (int i = 0; i < 10; i++) {
            ...
            for (int iChan = 0; iChan < 23; iChan++) {
                for (int i = 0; i < ADCSIZE; i++) {
                    ...
                    if (rawADC <= SATURATION_THRESHOLD_LOW || rawADC >= SATURATION_THRESHOLD_HIGH) {
                        ...
                        cout << "Saturation detected in event " << eventID << ", channel " << iChan << ", sample " << i << ", ADC = " << rawADC << endl;
                        break;
                    }
                }
